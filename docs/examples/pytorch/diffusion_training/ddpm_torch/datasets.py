...
def get_rocal_dataloader(
    ...
    data_path = os.path.join(root, 'celeba', 'img_align_celeba')

    pipe = Pipeline(batch_size=batch_size, num_threads=8, device_id=local_rank, seed=local_rank+10, rocal_cpu=rocal_cpu, tensor_dtype = types.FLOAT, tensor_layout=types.NCHW, prefetch_queue_depth = 7, mean=[0.5 * 255,0.5 * 255,0.5 * 255], std = [0.5 * 255,0.5 * 255,0.5 * 255], output_memory_type = types.DEVICE_MEMORY)
    with pipe:
        jpegs, labels = fn.readers.file(file_root=data_path)
        decode = fn.decoders.image(jpegs, output_type=types.RGB,
                                        file_root=data_path, shard_id=local_rank, num_shards=world_size, random_shuffle=True, max_decoded_width=178, max_decoded_height=218, last_batch_policy=types.LAST_BATCH_DROP)
        # cropped_images = fn.crop(decode, crop=(148, 148), crop_pos_x=15, crop_pos_y=40)
        # res = fn.resize(cropped_images, resize_width=64, resize_height=64, output_layout = types.NHWC, output_dtype = types.UINT8, interpolation_type=types.TRIANGULAR_INTERPOLATION)
        crop_resize = fn.resize_crop(decode, resize_width=64, resize_height=64, output_layout = types.NHWC, output_dtype = types.UINT8, interpolation_type=types.TRIANGULAR_INTERPOLATION, crop_w=148, crop_h=148, crop_pos_x=15, crop_pos_y=40)
    ...
    pipe.build()
    rocal_dataloader = ROCALClassificationIterator(pipe, device='gpu', device_id = local_rank)
    sampler = False

    return rocal_dataloader, batch_size
