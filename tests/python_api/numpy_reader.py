...
def main():
    ...
    with pipeline:
        ...
        data_sliced_output = fn.slice(data_output, anchor=anchor, shape=(1,128,128,128), rocal_tensor_output_type=types.FLOAT)
        label_sliced_output = fn.slice(label_output, anchor=anchor, shape=(1,128,128,128), rocal_tensor_output_type=types.UINT8)       
    ...
    valNumpyIteratorPipeline = ROCALNumpyIterator(pipeline1, device='cpu' if rocal_cpu else 'gpu', return_max_roi=True)
